...
import { deleteLightingEquipment } from "@/services/firebaseService";

interface LightingListProps {
  items: LightingEquipment[];
  onRefresh: () => void;
  onEdit: (data: LightingEquipment) => void;
}

const LightingList = ({ items, onRefresh, onEdit }: LightingListProps) => {
  ...
  const handleDelete = async (equipmentId: string) => {
    ...
    try {
      setLoading(true);
      await deleteLightingEquipment(equipmentId);
  ...
  if (items.length === 0) {
    return (
      <Card className="text-center p-8 animate-fade-in">
        <CardContent>
          <p className="text-muted-foreground mb-4">No lighting equipment has been added yet.</p>
        </CardContent>
      </Card>
    );
  }
